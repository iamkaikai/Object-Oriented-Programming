 ...
 */
public class BlobGUI1 extends DrawingGUI {
	private static final int width=800, height=600;	// size of the world

	private Blob blob;								// the blob
	
	public BlobGUI1() {
		super("SA-1 YENKAI HUANG: Animated Blob", width, height);
		
		// Create blob at the center.
		blob = new DrunkBlob(width/2, height/2);  // What happens if this isn't here? Try it so you see the symptom.
		blob.setVelocity(1, 1); // small steps
