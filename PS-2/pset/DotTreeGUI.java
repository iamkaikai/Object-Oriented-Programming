...
/**
 * @Authors: Christopher Kang and Kyle Huang
 * @Date: 04.24.2023
 */
...
public class DotTreeGUI extends DrawingGUI {
	...
	/**
	 * I inserted 7 dots near the center and I put a circle near the center which should capture all the dots.
	 * Then I added more dots around closer to the sides and put a larger circle that should
	 * only cover still the 7 circles in the center
	 */
	private void test2(){
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0,0,800,600);
		tree.insert(new Dot(401,301));
		tree.insert(new Dot(402,302));
		tree.insert(new Dot(403,303));
		tree.insert(new Dot(404,304));
		tree.insert(new Dot(405,305));
		tree.insert(new Dot(406,306));
		int x = tree.findInCircle(403, 303, 5).size();
		tree.insert(new Dot(100,100));
		tree.insert(new Dot(100,500));
		tree.insert(new Dot(500,100));
		tree.insert(new Dot(500,500));
		int y = tree.findInCircle(300, 300, 120).size();
		if (x == 7){
