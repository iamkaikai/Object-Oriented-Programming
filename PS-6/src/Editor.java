 ...
 * @author CBK, spring 2016 and Fall 2016, restructured Shape and some GUI
 */
...
public class Editor extends JFrame {	
	...
	private void handlePress(Point p) {
		...
		if (mode == Mode.DRAW){
			drawFrom = p;
			int x = (int) drawFrom.x;
			int y = (int) drawFrom.y;

			if (shapeType == "ellipse"){
				curr = new Ellipse(x, y, color);
			} else if (shapeType == "rectangle") {
				curr = new Rectangle(x, y, color);
			}else if (shapeType == "segment") {
				curr = new Segment(x, y, color);
			}else if (shapeType == "freehand") {
				ArrayList<Point> list = new ArrayList<Point>();
				curr = new Polyline(list, color);
			}
			repaint();
		}
...
//				moveFrom = p;  // Update the moveFrom point (get from GPT)
//				repaint();
//			}
//		}
	}
